...
class RobotControl
{
	...
	public void sampleControlMechanism(int barHeights[], int blockHeights[])
	{
		...
		for(int i =blockHeights.length-1 ;i>=0;i--)
		{
			...
			{
				System.out.println("djhas"+(currentBar-2));
				for(int k = currentBar-3;k<barHeights.length;k++)
				{
					System.out.println("nbda"+barHeights[k]);
					if(max < barHeights[k])
					{
						max = barHeights[k];
					}
			...
			System.out.println("max"+h+" " +blockHeights[i]);
			while (d > (h-max-1))
			{
				r.raise();
				d--;
			...
			*/
			...
			/*
			int max = barHeights[currentBar-3];
			for(int k = currentBar-2;k<barHeights.length;k++)
			{
				if(max < barHeights[k])
				{
			...
			while (d > (h-max-blockHeights[i]-1))
			{
				r.raise();
				d--;
			}
			...
			// dropping the block
			r.drop();
			System.out.println("i got dropped");
			if(blockHeights[i] == 1)
			{
